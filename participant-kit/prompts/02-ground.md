...
Goal: replace a component the model imagined with one that shipped.

One search per swap. Three swaps, three searches.

---

```
For each of my three region swaps below, first tell me in one line what you
would design with no reference at all - your own best guess. Then find a real,
shipped example on Mobbin for the same swap.
...
- your blind guess, from before you searched
- the structural logic of the shipped pattern in one or two lines: what is on
  screen at once, in what order, and what is deliberately absent
- one concrete thing the shipped pattern has that your blind guess didn't — an
  ordering, a density, an absence. If you can't name one, search again with a
  more specific query before moving on
- the app name and the mobbin_url
...
## Why we don't search Mobbin for "Crédit Mutuel"

Already checked, 16 August 2026: searching Mobbin for `Crédit Mutuel online banking account
screen` returns no Crédit Mutuel screens at all — Wise, Mercury, Square and Monarch instead.
No need to run it again on the day. The bank the "before" comes from isn't in the library; the
patterns the "after" draws on are. That gap is the demonstration.
