...
At the top of the file, put an HTML comment listing, for each swap: the region,
the mobbin_url you followed, and the one concrete thing that reference changed
versus what you would have built with no reference at all. If you can't name
that difference for a swap, it isn't grounded yet — go back to step 02.
```
...
## The failure to watch for: the beautiful reskin

The most common bad output is a new typeface, generous spacing, soft shadows, and the same
four-column table underneath doing the same job in the same order. It will look enormously
better than the baseline and it will have changed nothing.
...
## The other failure to watch for: the convergent default

Some swaps look grounded but aren't: a card stack, a feed grouped by day, a "safe to spend"
headline. These are common enough that a model reaches for them with no reference at all —
Mobbin or not.

Check it against the blind guess you wrote down in step 02. If the grounded version matches
that guess, the reference didn't earn its place. Go back and search again with something more
specific than the first result that looked plausible.
